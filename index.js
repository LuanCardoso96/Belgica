...
const dict = {
    pt: {
        ...
        "portfolio.title":"Nossas Referências","portfolio.desc":"Conheça alguns dos nossos projetos mais recentes.","portfolio.item1":"Residencial Atlas — 12 000 m²","portfolio.item2":"Ponte Leste — vãos de 40 m","portfolio.item3":"Parque Industrial Beta","portfolio.item4":"Hospital Central — fase 2","portfolio.item5":"Torre Norte — 22 andares","portfolio.item6":"Centro comercial Riviera","portfolio.item7":"Serviço Finalizado - Resultado",
    ...
    en: {
        ...
        "hero.kicker":"Subcontracting • Formwork • Rebar","hero.title":"Engineering that wins contracts and delivers quality","hero.lead":"We are a subcontracting company with <strong>200+ professionals</strong> focused on concrete structures, formwork and rebar. We serve general contractors with agility, safety and competitive cost.","hero.cta":"Request a quote","hero.secondary":"See projects",
    ...
    ja: {
        ...
        "hero.kicker":"下請け • 型枠 • 鉄筋","hero.title":"契約を獲得し品質を届けるエンジニアリング","hero.lead":"私たちは <strong>200名以上</strong> のプロで構成された下請け企業です。コンクリート構造・型枠・鉄筋に特化し、ゼネコン様へ機動力と安全性、競争力のあるコストで提供します。","hero.cta":"見積もり依頼","hero.secondary":"実績を見る",
...
function toggleTheme() {
    ...
    const currentTheme = root.getAttribute('data-theme') || 'dark'; // Default to dark
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(savedTheme === 'light') {
        ...
        const themeIcon = document.getElementById('theme-icon');
        if(themeIcon) themeIcon.innerHTML = '<path d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/>';
    }

    // Init Mobile Menu
    const mobileMenuBtn = document.getElementById('mobile-menu-toggle');
    if (mobileMenuBtn) mobileMenuBtn.addEventListener('click', toggleMobileMenu);
    document.querySelectorAll('.nav-list a').forEach(link => link.addEventListener('click', closeMobileMenu));
    document.addEventListener('click', (e) => {
        const mainNav = document.getElementById('primary-nav');
        const mobileMenuBtn = document.getElementById('mobile-menu-toggle');
        if (mainNav && mainNav.classList.contains('active') && !mainNav.contains(e.target) && !mobileMenuBtn.contains(e.target)) {
            closeMobileMenu();
        }
    });
